...
An Ansible collection (`community.mysql`) providing modules for managing MySQL and MariaDB databases. No roles exist — only modules and shared utilities.

## Development Environment

The collection must reside at `ansible_collections/community/mysql/` (relative to a directory on `ANSIBLE_COLLECTIONS_PATHS`) for imports to resolve correctly.
